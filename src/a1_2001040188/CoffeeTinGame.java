...
public class CoffeeTinGame {
    ...
    /**
     * returns as output an integer number randomly selected from the range [0,size of bean bag].
     * @requires: BeansBag not null /\ BeansBag.size > 0
     * @modifies:
     ...
     * The found bean is also removed from the array
     * @requires: BeansBag not null /\ BeanBag.size > 0
     * @modifies: BeansBag
     * @effects: after taking out two beans if
     * { they are the same color throw them both away put a Blue bean back in
     * } else { throw away the blue one put the green one back }
     */
    ...
    /**
     * takes as input the tin and two beans and updates tin accordingly.
     * In addition, this procedure must use getBean to obtain a desired bean from BeansBag.
     * @requires: BeansBag not null /\ BeanBag.size > 0
     * @modifies: BeansBag
     ...
     * Print all the beans before play Repeat implement the game until BeansBag = 1
     * Print the last bean.
     */
     ...
     * @requires:BeansBag not null /\ BeanBag.size > 0 /\ Tin not null /\ Tin > 0
     * @modifies: tin
